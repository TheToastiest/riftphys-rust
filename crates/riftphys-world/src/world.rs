    ...
    impl World {
        ...
        pub fn sweep_capsule(
            ...
            let dt = 1.0_f32; // we treat toi in [0,1] along [from -> to]
            let tip0  = from + Vec3::new(0.0,  half_height, 0.0);
            let base0 = from + Vec3::new(0.0, -half_height, 0.0);

            let mut best: Option<(f32, usize, Vec3, Vec3, bool)> = None;
            let eps = 1.0e-9;
            ...
            for (ci, c) in self.colliders.iter().enumerate() {
                ...
                let mut cand: Option<(f32, Vec3, Vec3, bool)> = None;

                // Check overlap at t=0
                let (p_seg, p_box) = closest_points_segment_aabb(
                    tip0,
                    base0,
                    c.aabb.min,
                    c.aabb.max,
                ...
                if dist < radius {
                    ...
                    cand = Some((0.0, point, n, true));
                } else {
                    // Sweep both end-spheres against this AABB and take earliest
                    let mut local_best: Option<(f32, Vec3)> = None;
                    for &center0 in &[tip0, base0] {
                        if let Some((t, n_raw)) = sweep_sphere_vs_aabb(center0, v, radius, &c.aabb, dt) {
                            let t_clamped = t.clamp(0.0, 1.0);
                            let n: Vec3 = n_raw.into();
                            match local_best {
                                None => local_best = Some((t_clamped, n)),
                                Some((bt, _)) if t_clamped < bt - eps => {
                                    local_best = Some((t_clamped, n));
                                }
                                _ => {}
                            }
                        }
                    ...
                    if let Some((t_local, n)) = local_best {
                        let center_at_hit = from + v * t_local;
                        let point = center_at_hit - n * radius;
                        cand = Some((t_local, point, n, false));
                    }
                }

                if let Some((t_cand, p_cand, n_cand, started)) = cand {
                    match best {
                        None => best = Some((t_cand, ci, p_cand, n_cand, started)),
                        Some((bt, bi, _, _, _)) => {
                            if (t_cand < bt - eps) || ((t_cand - bt).abs() <= eps && ci < bi) {
                                best = Some((t_cand, ci, p_cand, n_cand, started));
                            }
                        }
                    }
                }
            ...
            let (t, ci, point, normal, started) = best?;
            Some(SweepHit {
                body: self.colliders[ci].body,
                toi: t,
        ...
        pub fn apply_impulse(&mut self, id: BodyId, impulse: Vec3) {
            self.bodies.apply_impulse(id.0, impulse);
        }

        pub fn remove_body(&mut self, _id: BodyId) -> bool {
            // Placeholder: real deletion later when you care about recycling IDs.
            // For now, FFI remove() is a no-op.
            false
        }
    ...
    fn ray_aabb(origin: Vec3, dir: Vec3, aabb: &Aabb, max_dist: f32) -> Option<(f32, Vec3, Vec3)> {
        let eps = 1.0e-8;
    ...
    fn normal_from_aabb_point(aabb: &Aabb, p: Vec3) -> Vec3 {
        let eps = 1.0e-4;
