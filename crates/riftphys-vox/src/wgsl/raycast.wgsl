// Minimal raycast compute — no palette bindings in compute.
const EPS: f32 = 1e-7;
const T_PUSH : f32 = 1e-6;
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  var sx: i32;
  if (d.x >= 0.0) { sx = 1; } else { sx = -1; }
  var sy: i32;
  if (d.y >= 0.0) { sy = 1; } else { sy = -1; }
  var sz: i32;
  if (d.z >= 0.0) { sz = 1; } else { sz = -1; }
  ...
  if (abs(d.x) < 1e-12) {
    ...
    let edge_x = f32(cell.x + (0 + i32(sx > 0))) * cfg.params.x;
  ...
  if (abs(d.y) < 1e-12) {
    ...
    let edge_y = f32(cell.y + (0 + i32(sy > 0))) * cfg.params.x;
  ...
  if (abs(d.z) < 1e-12) {
    ...
    let edge_z = f32(cell.z + (0 + i32(sz > 0))) * cfg.params.x;
    tmaxz = (edge_z - p.z) / d.z + t;
  }

  var tdx: f32;
  if (abs(d.x) < 1e-12) { tdx = 1e30; } else { tdx = cfg.params.x / abs(d.x); }
  var tdy: f32;
  if (abs(d.y) < 1e-12) { tdy = 1e30; } else { tdy = cfg.params.x / abs(d.y); }
  var tdz: f32;
  if (abs(d.z) < 1e-12) { tdz = 1e30; } else { tdz = cfg.params.x / abs(d.z); }
  ...
  if (get_voxel(u32(cell.x), u32(cell.y), u32(cell.z))) {
    if (tmaxx + EPS <= tmaxy && tmaxx + EPS <= tmaxz) {
      if (sx > 0) { n = vec3<f32>( -1.0, 0.0, 0.0); } else { n = vec3<f32>(1.0, 0.0, 0.0); }
    } else if (tmaxy + EPS <= tmaxz) {
      if (sy > 0) { n = vec3<f32>( 0.0, -1.0, 0.0); } else { n = vec3<f32>( 0.0,1.0, 0.0); }
    } else {
      if (sz > 0) { n = vec3<f32>( 0.0, 0.0, -1.0); } else { n = vec3<f32>( 0.0, 0.0,1.0); }
    }
    if (t0 > tmax || t1 < t0) { hits[i].hit = 0u; return; }
      if (dot(n, d) > 0.0) { n = -n; }

    hits[i].hit = 1u; hits[i].t = t0;
